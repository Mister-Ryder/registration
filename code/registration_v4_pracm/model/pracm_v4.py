"""PRA-CM V4: relational, probabilistic and uncertainty-conditioned 3-D registration."""
...
from .correlation_v4 import AdaptiveCorrelation3D, CorrelationDistributionV4
from .encoder import EncodedPyramid, SharedStructuralResponseEncoder
from .posterior_solver import (
    PosteriorSolverOutput,
    UncertaintyConditionedPosteriorSolver3D,
)
...
class IterationOutput3D:
    ...
    distribution: CorrelationDistributionV4
...
class LevelOutput3D:
    ...
    solver_correction: torch.Tensor
...
class PRACM3D(nn.Module):
    """One architecture with three frozen mechanism switches via model.variant."""
    ...
    def __init__(self, config: Optional[ModelConfig] = None) -> None:
        ...
        self.response_gate = DirectedResponseCompatibility(
            ...
            self.config.acquisition_logit_bound,
        )
    ...
    @property
    def uses_adaptive_posterior(self) -> bool:
        return self.config.variant in {"v4b", "v4_full"}

    @property
    def uses_global_solver(self) -> bool:
        return self.config.variant == "v4_full"
    ...
    def _feature_spacing(
        spacing_dzyx: torch.Tensor,
        native_size,
        feature_size,
    ) -> torch.Tensor:
        ratios = [
            (native_size[i] - 1) / (feature_size[i] - 1)
            if native_size[i] > 1 and feature_size[i] > 1
            else 1.0
            for i in range(3)
        ]
        return spacing_dzyx * spacing_dzyx.new_tensor(ratios)[None]
    ...
    def auxiliary_predictions(
        ...
        *,
        ...
        reversed_structural = GradientReversal.apply(structural_summary, adversarial_scale)
    ...
    def forward(
        ...
        *,
        ...
        batch = fixed.shape[0]
        if spacing_dzyx is None:
            spacing = fixed.new_ones((batch, 3))
        else:
            spacing = torch.as_tensor(spacing_dzyx, device=fixed.device, dtype=fixed.dtype)
            if spacing.ndim == 1:
                spacing = spacing[None].expand(batch, -1)
            if spacing.shape != (batch, 3) or (spacing <= 0).any():
                raise ValueError("spacing_dzyx must be positive [B,3].")
        ...
        level_outputs = []
        for level_index, (level, radius, iteration_count) in enumerate(zip(
            ...
            feature_spacing = self._feature_spacing(
                spacing, fixed.shape[-3:], fixed_s.shape[-3:]
            )
            correlation = AdaptiveCorrelation3D(
                radius,
                self.response_gate,
                temperature=self.config.correlation_temperature,
                chunk_size=self.config.candidate_chunk_size,
                align_corners=self.config.align_corners,
                adaptive_support=self.uses_adaptive_posterior,
                minimum_support_mm=self.config.minimum_search_support_mm[level_index],
                maximum_support_mm=self.config.maximum_search_support_mm[level_index],
                mode_centered=self.uses_adaptive_posterior,
                mode_radius=self.config.posterior_mode_radius,
                topk=self.config.posterior_topk,
            )
            hidden = None
            iterations = []
            ...
            for iteration in range(iteration_count):
                ...
                distribution = correlation(
                    ...
                    fixed_phase_ids,
                    moving_phase_ids,
                    feature_spacing_dzyx=feature_spacing,
                ...
                warped_moving_s = warp(
                    moving_s, base_flow, align_corners=self.config.align_corners
                )
                residual = (
                    distribution.mode_residual
                    if self.uses_adaptive_posterior
                    else distribution.mean_residual
                )
                update = self.updater(
                    fixed_s,
                    warped_moving_s,
            ...
            solver_correction = flow.new_zeros((batch, 1, *flow.shape[-3:]))
            if self.uses_global_solver:
                solver = UncertaintyConditionedPosteriorSolver3D(
                    iterations=(
                        self.config.solver_iterations_train
                        if self.training
                        else self.config.solver_iterations_inference
                    ),
                    data_sigma=self.config.solver_data_sigma,
                    spatial_weight=self.config.solver_spatial_weight,
                )
                solver_inputs = (
                    iterations[-1].base_flow,
                    final_distribution.topk_residuals,
                    final_distribution.topk_probabilities,
                    final_distribution.entropy,
                )
                if self.training and torch.is_grad_enabled():
                    def solve_tensors(base, hypotheses, probabilities, entropy):
                        current = solver(base, hypotheses, probabilities, entropy)
                        return (
                            current.flow,
                            current.variance,
                            current.data_flow,
                            current.correction_magnitude,
                        )

                    solved = PosteriorSolverOutput(
                        *checkpoint(
                            solve_tensors,
                            *solver_inputs,
                            use_reentrant=False,
                    ...
                    solved = solver(*solver_inputs)
                flow = solved.flow
                variance = variance + solved.variance
                solver_correction = solved.correction_magnitude
            previous_entropy = final_distribution.entropy
            level_outputs.append(
                LevelOutput3D(
                    ...
                    final_distribution.support_radius_mm,
                    solver_correction,
                    tuple(iterations),
                )
            )
        ...
        gate = F.interpolate(final.expected_gate, **scalar_kwargs).clamp_min(0)
        coverage = F.interpolate(final.candidate_coverage, **scalar_kwargs).clamp(0, 1)
        support = F.interpolate(final.support_radius_mm, **scalar_kwargs).clamp_min(0)
        solver_correction = F.interpolate(final.solver_correction, **scalar_kwargs)
